...
(function (TSOS) {
    var DeviceDriverFileSystem = (function () {
        ...
        DeviceDriverFileSystem.prototype.createStartFile = function () {
            ...
            //$("#fs" + tsbKey + " td:nth-child(2)").html(startFileData.substr(0, 4));
            //$("#fs" + tsbKey + " td:nth-child(3)").html(startFileData.substr(4));
        };
        ...
        DeviceDriverFileSystem.prototype.createFile = function (fileName) {
            ...
            //$("#fs" + tsbKey + " td:nth-child(2)").html(fileName.substr(0, 4));
            //$("#fs" + tsbKey + " td:nth-child(3)").html(fileName.substr(4));
            var initialData = this.createInitialData();
            initialData = "1---" + initialData.substr(4);
            sessionStorage.setItem(dataTSBKey, initialData);
            //$("#fs" + dataTSBKey + " td:nth-child(2)").html(initialData.substr(0, 4));
            //$("#fs" + dataTSBKey + " td:nth-child(3)").html(initialData.substr(4));
            return true;
        };
        ...
        DeviceDriverFileSystem.prototype.deleteFile = function (startingTSB, fileName) {
            ...
            //$("#fs" + dirTSB + " td:nth-child(2)").html(initialData.substr(0, 4));
            //$("#fs" + dirTSB + " td:nth-child(3)").html(initialData.substr(4));
            //
            //while(nextTSB != "---"){
            //    sessionStorage.setItem(nextTSB, initialData);
            //    $("#fs" + nextTSB + " td:nth-child(2)").html(initialData.substr(0, 4));
            //    $("#fs" + nextTSB + " td:nth-child(3)").html(initialData.substr(4));
            //    nextTSB = nextNextTSB;
            //    if(nextTSB == "---"){
            //        break;
            //    }
            ...
            //$("#fs" + startingTSB.substr(1) + " td:nth-child(2)").html(initialData.substr(0, 4));
            //$("#fs" + startingTSB.substr(1) + " td:nth-child(3)").html(initialData.substr(4));
            return true;
        };
        ...
        DeviceDriverFileSystem.prototype.initializeFileSystemToScreen = function () {
            ...
            for (var track = 0; track < this.tracks; track++) {
                for (var sector = 0; sector < this.sectors; sector++) {
                    for (var block = 0; block < this.blocks; block++) {
                        metaData = this.createInitialData().substr(0, 4);
                        data = this.createInitialData().substr(4);
                        tr = document.createElement("tr");
                        tr.id = "tsb" + this.makeKey(track, sector, block);
